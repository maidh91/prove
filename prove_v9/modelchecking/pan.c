/*** Generated by Spin Version 5.2.0 -- 2 May 2009 ***/
/*** From source: verify.c ***/
...
Again:
	...
	{	trpt->b_pno = now._last + BASE;
		trpt->bcs = B_PHASE1;
	#ifdef VERBOSE
		printf("%3d: BCS phase 1 proc %d limit %d\n",
...
c_switch:	/* jumps here with bcs == B_PHASE2 with or wo B_FORCED added */
	#ifdef VERBOSE
		printf("%3d: BCS c_switch phase=%d pno=%d [forced %d]\n",
...
r_switch:
	#ifdef VERBOSE
		printf("%3d: P_RAND r_switch p_skip=%d p_left=%d\n",
	...
	for ALL_P
	{
	...
	#ifdef VERBOSE
		printf("%3d: P_RAND skipping %d [new p_skip=%d p_left=%d]\n",
		...
		{
	#ifdef VERBOSE
		printf("%3d: P_RAND done at %d\n", depth, II);
	#endif
			break;	/* done */
		}
	#ifdef VERBOSE
		printf("%3d: P_RAND explore %d [p_left=%d]\n",
		...
		{
	#ifdef VERBOSE
		printf("%3d: BCS NotPre II=%d bcs=%d pno=%d [forced %d]\n",
		...
		printf("%3d: BCS IsPre II=%d bcs=%d pno=%d [forced %d]\n",
			depth, II, trpt->bcs, trpt->b_pno, (trpt->bcs&B_FORCED)?1:0);
	#endif
		...
		{	if (trpt->b_pno == II)	/* was already done in phase 1 */
			{
	#ifdef VERBOSE
		printf("%3d: BCS NoRepeat II=%d bcs=%d pno=%d [forced %d]\n",
			depth, II, trpt->bcs, trpt->b_pno, (trpt->bcs&B_FORCED)?1:0);
	#endif
				continue;
			}
			...
			{
	#ifdef VERBOSE
		printf("%3d: BCS Bound II=%d bcs=%d pno=%d [forced %d]\n",
			depth, II, trpt->bcs, trpt->b_pno, (trpt->bcs&B_FORCED)?1:0);
	#endif
...
P999:			/* jumps here when move succeeds */
	...
	#ifdef VERBOSE
		printf("%3d: up sched count to %d\n", depth, (trpt+1)->sched_limit);
	#endif
			} else
			{	(trpt+1)->sched_limit = trpt->sched_limit;
	#ifdef VERBOSE
		printf("%3d: keep sched count at %d\n", depth, (trpt+1)->sched_limit);
	#endif
			}
#endif
			if (boq == -1)
...
R999:			/* jumps here when done */
	...
	#ifdef VERBOSE
		printf("%3d: P_RAND -- explore remainder\n", depth);
	#endif
		goto r_switch; /* explore the remaining procs */
	} else
	{
	#ifdef VERBOSE
		printf("%3d: P_RAND -- none left\n", depth);
	#endif
	}
#endif
#ifdef BCS
	...
	{	trpt->bcs = B_PHASE2;	/* start 2nd phase */
		if (_n == 0)	/* pre-move was unexecutable */
		{	trpt->bcs |= B_FORCED; /* forced switch */
		}
	#ifdef VERBOSE
		printf("%3d: BCS move to phase 2, _n=%d\n", depth, _n);
	#endif
		goto c_switch;
	}
